...
const _devAccount = JSON.parse(
  fs.readFileSync(
    path.resolve(
      "./metro-app-dev-f4ab1-firebase-adminsdk-dth49-e6392ae8ff.json"
    ),
    "utf-8"
  )
);
...
const senryoUat = JSON.parse(
  fs.readFileSync(
    path.resolve("./senryo-revamp-uat-firebase-adminsdk-eldxm-965282d484.json"),
    "utf-8"
  )
);
...
const _imagePath =
  "https://media-mbst-pub-ue1.s3.amazonaws.com/creatr-uploaded-images/2019-09/08495a20-e329-11e9-a793-fa17a931f371";
...
const sendPn = async () => {
  try {
    ...
    console.log(
      `${title} (${response.successCount}/${response.responses.length})`
    );
  } catch (error) {
